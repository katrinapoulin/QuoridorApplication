Feature: Jump to final position
As a player using replay mode, I wish to scroll fast to the very beginning of the game.

	  Background: 
    Given The game is in replay mode

  Scenario Outline: Jump to final
    Given The following moves have been played in game:
      | mv | rnd | move |
      |  1 |   1 | e8   |
      |  1 |   2 | e2   |
      |  2 |   1 | e7   |
      |  2 |   2 | e3   |
      |  3 |   1 | e3h  |
      |  3 |   2 | e8h  |
      |  4 |   1 | d3v  |
      |  4 |   2 | f2   |
    And The next move is <movno>.<rndno>
    When Jump to final position is initiated
    Then The next move will be <nmov>.<nrnd>
    And White player's position will be (<wrow>,<wcol>)
    And Black player's position will be (<brow>,<bcol>)
    And White has <wwallno> on their stock
    And Black has <bwallno> on their stock

    Examples: 
      | movno | rndno | nmov | nrnd | wrow | wcol | wwallno | brow | bcol | bwallno |
      |     1 |     1 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      |     1 |     2 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      |     2 |     1 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      |     2 |     2 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      |     3 |     1 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      |     4 |     2 |    5 |    1 |    7 |    5 |     8 	|    3 |    6 |       9 |
      